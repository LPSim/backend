...
from ....action import (
    ActionTypes, ChangeObjectUsageAction, MakeDamageAction, RemoveObjectAction
)
from ....event import (
...
class PlungingIceShards(ElementalBurstBase, AOESkillBase):
    ...
    def get_actions(self, match: Match) -> List[Actions]:
        return super().get_actions(match, [
            self.create_summon('Piercing Iceridge')
        ])
...
class CryoHypostasis_4_4(CharactorBase):
    ...
    max_charge: int = 2
    skills: List[
        IcespikeShot | IceRingWaltz | PlungingIceShards
        | CryocrystalCore
    ] = []
