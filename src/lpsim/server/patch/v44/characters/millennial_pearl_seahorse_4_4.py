...
class FontemerPearl_4_4(DefendCharacterStatus):
    ...
    def renew(self, obj: 'FontemerPearl_4_4') -> None:
        ...
        self.round_extra_usage = max(
            self.round_extra_usage, obj.round_extra_usage)
        if obj.desc == 'talent':
            self.desc = 'talent'
    ...
    def value_modifier_DAMAGE_DECREASE(
        self, value: DamageDecreaseValue, match: Match, 
    ...
    def get_actions(
        ...
        if pearl_status is not None:
            ...
            ret += [ChangeObjectUsageAction(
                object_position = pearl_status.position,
                change_usage = 1
            )]
        return ret
...
class MillennialPearlSeahorse_4_4(CharacterBase):
    ...
    max_charge: int = 2
    skills: List[
        PhysicalNormalAttackBase | SwirlingSchoolOfFish | FontemerHoarthunder
        | PearlArmor
    ] = []
...
register_class(
    FontemerPearl_4_4 | ResonantCoralOrb_4_4 | MillennialPearlSeahorse_4_4
