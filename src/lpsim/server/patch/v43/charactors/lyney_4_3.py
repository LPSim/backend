...
from ....charactor.charactor_base import (
    ...
    ElementalSkillBase, PhysicalNormalAttackBase, SkillBase, SkillTalent
)
from .....utils.desc_registry import DescDictType


class PropSurplus_4_3(CharactorStatusBase):
...
class ConclusiveOvation_4_3(SkillTalent):
    ...
    charactor_name: Literal["Lyney"] = "Lyney"
    version: Literal['4.3'] = '4.3'
    ...
    def value_modifier_DAMAGE_INCREASE(
        ...
        if value.target_position.player_idx == self.position.player_idx:
            # self target self
            return value
        target: CharactorBase = match.get_object(
            value.target_position)  # type: ignore
        ...
        if (
            source is None or source.name not in ['Lyney', 'Grin-Malkin Hat']
        ):
...
class Lyney_4_3(CharactorBase):
    ...
    max_charge: int = 2
    skills: List[
        PhysicalNormalAttackBase | PropArrow | ElementalSkillBase 
        | WondrousTrickMiracleParade
    ] = []
    faction: List[FactionType] = [
        FactionType.FONTAINE, FactionType.FATUI, FactionType.ARKHE_PNEUMA
    ]
...
desc: Dict[str, DescDictType] = {
    ...
    "SKILL_Lyney_NORMAL_ATTACK/Card Force Translocation": {
        "names": {
            "en-US": "Card Force Translocation",
            "zh-CN": "迫牌易位式"
        },
        "descs": {
            "4.3": {
...
register_class(
    GrinMalkinHat_4_3 | PropSurplus_4_3 | ConclusiveOvation_4_3 | Lyney_4_3,
    desc
)
