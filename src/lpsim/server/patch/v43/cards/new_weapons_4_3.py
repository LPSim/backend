...
from ....consts import DamageType, IconType, ObjectPositionType, SkillType, WeaponType
...
class BeaconOfTheReedSea_4_3(RoundEffectWeaponBase):
    ...
    def event_handler_SKILL_END(
        ...
        if not self.position.check_position_valid(
            ...
            return []
        skill: SkillBase = match.get_object(event.action.position)  # type: ignore
        assert skill is not None
...
class LightOfFoliarIncision_4_3(RoundEffectWeaponBase):
    ...
    def event_handler_SKILL_END(
        ...
        return [
            CreateDiceAction(
                player_idx=self.position.player_idx, number=1, different=True
            )
        ]
...
desc: Dict[str, DescDictType] = {
    "WEAPON/Lost Prayer to the Sacred Winds": {
        "names": {"en-US": "Lost Prayer to the Sacred Winds", "zh-CN": "四风原典"},
        "descs": {
            "4.3": {
    ...
    "WEAPON/Tulaytullah's Remembrance": {
        "names": {"en-US": "Tulaytullah's Remembrance", "zh-CN": "图莱杜拉的回忆"},
        "descs": {
            "4.3": {
    ...
    "CHARACTER_STATUS/Beacon of the Reed Sea": {
        "names": {"en-US": "Beacon of the Reed Sea", "zh-CN": "苇海信标"},
        "descs": {
            "4.3": {
    ...
    "WEAPON/Primordial Jade Winged-Spear": {
        "names": {"en-US": "Primordial Jade Winged-Spear", "zh-CN": "和璞鸢"},
        "descs": {
            "4.3": {
    ...
    "WEAPON/Light of Foliar Incision": {
        "names": {"en-US": "Light of Foliar Incision", "zh-CN": "裁叶萃光"},
        "descs": {
            "4.3": {
...
register_class(
    LostPrayerToTheSacredWinds_4_3
    | TulaytullahsRemembrance_4_3
    | BeaconOfTheReedSea_4_3
    | PrimordialJadeWingedSpear_4_3
    | LightOfFoliarIncision_4_3
    | BeaconOfTheReedSeaStatus_4_3,
    desc,
)
