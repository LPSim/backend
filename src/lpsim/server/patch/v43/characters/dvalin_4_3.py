...
class TotalCollapse_4_3(UsageCharacterStatus):
...
class PerpetualCleansingStatus_4_3(PrepareCharacterStatus):
...
class UltimateCleansingStatus_4_3(PrepareCharacterStatus):
...
class PerpetualCleansing(ElementalSkillBase):
    ...
    def get_actions(self, match: Match) -> List[Actions]:
        ...
        target = table.next_character_idx()
        if target is None:
            target = table.active_character_idx
        return [
            MakeDamageAction(
                damage_value_list=[
                    DamageValue(
                        ...
                        target_position=table.characters[target].position,
                        damage=self.damage,
...
class UltimateCleansing(ElementalSkillBase):
    ...
    def get_actions(self, match: Match) -> List[Actions]:
        table = match.player_tables[1 - self.position.player_idx]
        target = table.previous_character_idx()
        if target is None:
            target = table.active_character_idx
        return [
            MakeDamageAction(
...
class CaelestinumFinaleTermini(ElementalBurstBase):
    ...
    damage_type: DamageElementalType = DamageElementalType.ANEMO
    cost: Cost = Cost(
        elemental_dice_color=DieColor.ANEMO, elemental_dice_number=4, charge=2
    )
    ...
    def get_actions(self, match: Match) -> List[Actions]:
        ...
        for character in table.characters:
            if table.active_character_idx == character.position.character_idx:
                continue
...
class RendingVortex_4_3(SkillTalent):
    ...
    def event_handler_CREATE_OBJECT(
        ...
        fake_event = RemoveObjectEventArguments(
            ...
            object_type=ObjectType.CHARACTER_STATUS,
        )
    ...
    def event_handler_REMOVE_OBJECT(
        ...
        self is in character, create Total Collapse on next standby character.
        """
        ...
        if not self.position.check_position_valid(
            ...
            # not from opposite active character, or self not equipped
            return []
        # is opposite active, append Total Collapse to next standby character
        table = match.player_tables[1 - self.position.player_idx]
        target = table.next_character_idx()
        if target is None:
            ...
            return []
        self.round_usage -= 1
        target_character = table.characters[target]
        return [
            CreateObjectAction(
                object_name="Total Collapse",
                object_position=target_character.position.set_area(
                    ObjectPositionType.CHARACTER_STATUS
                ),
                object_arguments={},
            )
...
class Dvalin_4_3(CharacterBase):
    name: Literal["Dvalin"]
    version: Literal["4.3"] = "4.3"
...
desc: Dict[str, DescDictType] = {
    ...
    "CHARACTER_STATUS/Total Collapse": {
        "names": {"en-US": "Total Collapse", "zh-CN": "坍毁"},
        "descs": {
            "4.3": {
    ...
    "SKILL_Dvalin_ELEMENTAL_SKILL/Dvalin's Cleansing": {
        "names": {"en-US": "Dvalin's Cleansing", "zh-CN": "风龙涤流"},
        "descs": {
            "4.3": {
    ...
    "CHARACTER_STATUS/Perpetual Cleansing": {
        "names": {"en-US": "Perpetual Cleansing", "zh-CN": "长延涤流"},
        "descs": {
            "4.3": {
    ...
    "CHARACTER_STATUS/Ultimate Cleansing": {
        "names": {"en-US": "Ultimate Cleansing", "zh-CN": "终幕涤流"},
        "descs": {
            "4.3": {
    ...
    "SKILL_Dvalin_ELEMENTAL_BURST/Caelestinum Finale Termini": {
        "names": {"en-US": "Caelestinum Finale Termini", "zh-CN": "终天闭幕曲"},
        "descs": {
            "4.3": {
    ...
    "TALENT_Dvalin/Rending Vortex": {
        "names": {"en-US": "Rending Vortex", "zh-CN": "毁裂风涡"},
        "descs": {
            "4.3": {
...
register_class(
    TotalCollapse_4_3
    | PerpetualCleansingStatus_4_3
    | UltimateCleansingStatus_4_3
    | RendingVortex_4_3
    | Dvalin_4_3,
    desc,
)
