...
class CryoCicins_4_1(AttackerSummonBase):
    ...
    def event_handler_SKILL_END(
        ...
        if event.action.skill_type == SkillType.NORMAL_ATTACK:
            ...
            if self.usage + 1 > self.max_usage:
                self.over_maximum = True
        # if over_maximum and character has talent, 2 damage to opposite active
        if self.over_maximum and character.talent is not None:
    ...
    def event_handler_RECEIVE_DAMAGE(
        ...
        if (
            event.final_damage.target_position.player_idx != self.position.player_idx
...
def find_cryo_cicins(position: ObjectPosition, match: Any, usage: int) -> Any | None:
    """
    ...
    for summon in summons:
        if summon.name == "Cryo Cicins" and summon.usage + usage > summon.max_usage:
            return summon
    return None
...
class CicinsColdGlare_3_7(SkillTalent):
    ...
    character_name: Literal["Fatui Cryo Cicin Mage"] = "Fatui Cryo Cicin Mage"
    cost: Cost = Cost(elemental_dice_color=DieColor.CRYO, elemental_dice_number=3)
...
class FatuiCryoCicinMage_4_1(CharacterBase):
    ...
    def _init_skills(self) -> None:
        self.skills = [CicinIcicle(), MistySummons(), BlizzardBranchBlossom()]
