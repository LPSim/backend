        ...
        for type in base_class.__args__:  # type: ignore
            ...
            try:
                return _parse_object(
                    _class_dict[type], _class_version_dict[type], name, version, args
                )
            except KeyError:
                continue
        raise AssertionError(f"Cannot find class {name} in union type {base_class}")
    else:
...
def get_class_list_by_base_class(
    ...
    for type in base_class_list:
        ...
        for name in names:
            ...
            try:
                _ = _get_class(
                    _class_dict[type], _class_version_dict[type], name, version
                )
                # get class success
                result_set.add(name)
            except KeyError:
