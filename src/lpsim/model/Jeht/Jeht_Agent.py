...
class JehtAgent(AgentBase):
    ...
    def generate_response(self, match: Match) -> Responses | None:
        req_names = list(
            set([x.name for x in match.requests if x.player_idx == self.player_idx])
        )
        req_names.sort()
        if len(req_names) == 0:
            return None
    ...
    def resp_switch_card(self, req: SwitchCardRequest) -> SwitchCardResponse:
        ...
        for i in range(len(req.card_names)):
            if self.random() < 0.5:
                card_idxs.append(i)
        return SwitchCardResponse(request=req, card_idxs=card_idxs)

    def resp_choose_character(
        self, req: ChooseCharacterRequest
    ) -> ChooseCharacterResponse:
        """
    ...
    def resp_reroll_dice(self, req: RerollDiceRequest) -> RerollDiceResponse:
        ...
        return RerollDiceResponse(request=req, reroll_dice_idxs=reroll_dice_idxs)

    def resp_elemental_tuning(
        self, req: ElementalTuningRequest
    ) -> ElementalTuningResponse:
        """
        Randomly choose a subset of dice.
        """
        return ElementalTuningResponse(
            request=req,
            dice_idx=req.dice_idxs[int(self.random() * len(req.dice_idxs))],
            card_idx=req.card_idxs[int(self.random() * len(req.card_idxs))],
        )

    def resp_switch_character(
        self, req: SwitchCharacterRequest
    ) -> SwitchCharacterResponse:
        """
        Randomly choose a character.
        """
    ...
    def resp_use_skill(self, req: UseSkillRequest) -> UseSkillResponse:
        """
        ...
        ele_dice_idxs = [
            num
            for num, color in enumerate(req.dice_colors)
            if color == cost.elemental_dice_color or color == DieColor.OMNI
        ]
        other_dice_idxs = [
            x for x in range(len(req.dice_colors)) if x not in ele_dice_idxs
        ]
        selected = []
        ...
        return UseSkillResponse(request=req, dice_idxs=selected)

    def resp_use_card(self, req: UseCardRequest) -> UseCardResponse:
        """
        ...
        if cost.same_dice_number > 0:
            ...
            for k in keys:
                ...
                if v >= cost.same_dice_number:
                    ...
                    k != DieColor.OMNI
                    and v + color_map[DieColor.OMNI] >= cost.same_dice_number
                ):
                    available_keys.append(k)
        ...
        target = None
        if len(req.targets):
            target = req.targets[int(self.random() * len(req.targets))]
