...
import os
from tests.utils_for_test import (  # noqa: E402
...
# for powershell: $env:PYTHONPATH = "C:/Users/zyr17/Documents/Projects/GITCG"
# for bash: export PYTHONPATH="/mnt/c/Users/zyr17/Documents/Projects/GITCG"
...
def template():
    ...
    if match.config.initial_dice_number == 16:
        set_16_omni(match)
    match.start()
    match.step()
    new_commands = [[], []]
        ...
        for tnum, table in enumerate(match.player_tables):
            ...
            for cnum, c in enumerate(table.characters):
                ...
                if c.is_alive and 12 in enable:
                    nc.append(cele_str)
                # 13 for usage of equips
                eq_usage_str = f"TEST 13 p{tnum}c{cnum} "
                if c.is_alive and 13 in enable:
                    if c.weapon is None:
                        eq_usage_str += "N"
                    else:
                        eq_usage_str += str(
            ...
            # 14 for summon desc
            summon_desc_str = f"TEST 14 p{tnum} |"
            for summon in table.summons:
                summon_desc_str += summon.desc + "|"
            if 14 in enable:
                nc.append(summon_desc_str)
